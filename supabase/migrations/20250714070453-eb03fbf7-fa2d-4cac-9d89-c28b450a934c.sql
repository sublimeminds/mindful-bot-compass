...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.notification_routing_rules LIMIT 1) THEN
    INSERT INTO public.notification_routing_rules (notification_type, priority, channels, conditions) VALUES
    ('crisis_detected', 'high', ARRAY['whatsapp', 'push', 'email', 'in_app'], '{"immediate": true}'::jsonb),
    ('emergency_contacts', 'high', ARRAY['whatsapp', 'email'], '{"notify_contacts": true}'::jsonb),
    ('safety_plan_reminder', 'high', ARRAY['push', 'in_app'], '{"urgent": true}'::jsonb),
    ('session_reminder', 'medium', ARRAY['push', 'in_app'], '{"timing": "15_minutes_before"}'::jsonb),
    ('milestone_achieved', 'medium', ARRAY['push', 'in_app'], '{"celebratory": true}'::jsonb),
    ('mood_check', 'low', ARRAY['in_app'], '{"gentle": true}'::jsonb),
    ('technique_suggestion', 'medium', ARRAY['in_app'], '{"contextual": true}'::jsonb),
    ('breathing_reminder', 'medium', ARRAY['push', 'in_app'], '{"stress_triggered": true}'::jsonb),
    ('goal_progress', 'low', ARRAY['in_app'], '{"encouraging": true}'::jsonb),
    ('new_content', 'low', ARRAY['in_app'], '{"personalized": true}'::jsonb),
    ('session_followup', 'medium', ARRAY['in_app'], '{"post_session": true}'::jsonb),
    ('streak_celebration', 'medium', ARRAY['push', 'in_app'], '{"celebratory": true}'::jsonb),
    ('professional_escalation', 'high', ARRAY['email', 'in_app'], '{"professional": true}'::jsonb),
    ('inactive_user', 'low', ARRAY['email', 'push'], '{"re_engagement": true}'::jsonb),
    ('community_invite', 'low', ARRAY['in_app'], '{"social": true}'::jsonb),
    ('challenge_invitation', 'low', ARRAY['push', 'in_app'], '{"gamification": true}'::jsonb),
    ('cross_platform_sync', 'low', ARRAY['in_app'], '{"technical": true}'::jsonb),
    ('integration_status', 'medium', ARRAY['in_app'], '{"system": true}'::jsonb),
    ('backup_reminder', 'low', ARRAY['in_app'], '{"maintenance": true}'::jsonb);
  END IF;
END $$;