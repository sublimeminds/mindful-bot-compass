-- First, safely handle existing foreign key constraints
-- Drop the foreign key constraints temporarily to allow data cleanup
ALTER TABLE IF EXISTS public.therapist_assessments DROP CONSTRAINT IF EXISTS therapist_assessments_selected_therapist_id_fkey;
ALTER TABLE IF EXISTS public.therapist_selections DROP CONSTRAINT IF EXISTS therapist_selections_therapist_id_fkey;

-- Clear tables that reference therapist_personalities to avoid FK violations
DELETE FROM public.therapist_assessments WHERE selected_therapist_id IS NOT NULL;
DELETE FROM public.therapist_selections WHERE therapist_id IS NOT NULL;

-- Now safely clear and rebuild therapist_personalities
DELETE FROM public.therapist_personalities;

-- Add new columns to therapist_personalities table
...
-- Add unique constraints (but drop first if they exist)
ALTER TABLE public.therapist_personalities DROP CONSTRAINT IF EXISTS unique_identifier_constraint;
ALTER TABLE public.therapist_personalities DROP CONSTRAINT IF EXISTS unique_avatar_image;
ALTER TABLE public.therapist_personalities DROP CONSTRAINT IF EXISTS unique_voice_id;

ALTER TABLE public.therapist_personalities 
ADD CONSTRAINT unique_identifier_constraint UNIQUE (unique_identifier),
ADD CONSTRAINT unique_avatar_image UNIQUE (avatar_image_url),
ADD CONSTRAINT unique_voice_id UNIQUE (elevenlabs_voice_id);

-- Insert comprehensive therapist personalities with unique profiles
