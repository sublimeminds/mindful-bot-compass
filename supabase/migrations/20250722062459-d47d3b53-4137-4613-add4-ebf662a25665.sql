...
-- First, let's create the missing tables with the correct schema
...
CREATE TABLE IF NOT EXISTS public.navigation_menu_categories (
  id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.navigation_menu_items (
  id TEXT PRIMARY KEY,
  menu_id TEXT NOT NULL REFERENCES public.navigation_menus(id) ON DELETE CASCADE,
  category_id TEXT REFERENCES public.navigation_menu_categories(id) ON DELETE SET NULL,
  title TEXT NOT NULL,
...
-- Clear existing data and insert the complete menu structure
...
-- Insert all navigation menu items with the correct schema
