...
interface State {
  ...
  isPlaying: boolean;
  displayedMessages: typeof demoConversation;
  isTyping: boolean;
}
...
class NativeChatDemo extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      currentMessage: 0,
      isPlaying: props.autoStart || false,
      displayedMessages: [],
      isTyping: false
    };
  }
  ...
  private startDemo = () => {
    if (this.state.currentMessage >= demoConversation.length) return;

    const timer = setTimeout(() => {
      if (this.state.currentMessage === 0) {
        this.setState({
          displayedMessages: [demoConversation[0]],
          currentMessage: 1
        });
      } else {
        ...
        const typingTimer = setTimeout(() => {
          this.setState(prevState => ({
            displayedMessages: [...prevState.displayedMessages, demoConversation[prevState.currentMessage]],
            isTyping: false,
    ...
    }, this.state.currentMessage === 0 ? 500 : demoConversation[this.state.currentMessage - 1]?.delay || 2000);
  ...
  private toggleDemo = () => {
    if (this.state.displayedMessages.length === 0) {
      this.resetDemo();
    } else {
  ...
  render() {
    const { currentMessage, isPlaying, displayedMessages, isTyping } = this.state;
